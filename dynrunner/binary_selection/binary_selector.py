"""Asm-binary selection filters + walk helpers, vendored out of the
framework in commit 6c65bb7. Compiles a `SelectionConfig` (see
`.selection_args`) into reusable filters; `match_filename` parses one
filename and returns a `BinaryIdentifier` if it clears the filters."""
import os
import re
