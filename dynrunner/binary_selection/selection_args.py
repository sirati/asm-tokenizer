"""Argparse + post-parse processing for asm-binary selection flags,
vendored out of the framework in commit 6c65bb7. The framework now
only owns `--source` / `--output` / `--list-files`; everything else
on the corpus-shape filter axis (`--platform`, `--compiler`,
`--compiler-versions`, `--opt`, `--file-format`, regex overrides,
`--debugs`, `--exclude-subfolder`) is asm-specific and lives here.

Each TaskDefinition that needs these flags calls
`add_asm_selection_arguments(parser)` from its `add_task_arguments`
implementation, then uses `process_selection_arguments(args)` inside
`discover_items` to compile the `SelectionConfig` consumed by
`compile_selection_filters` (see `.binary_selector`)."""
import argparse
import re
import sys
...
def add_asm_selection_arguments(parser: argparse.ArgumentParser) -> None:
    """Add the asm-specific binary-selection flags to a TaskDefinition's
    parser. The framework still owns `--source` / `--output` /
    `--list-files`; this layer adds `--platform`, `--compiler`,
    `--compiler-versions`, `--opt`, `--file-format`, regex overrides,
    `--debugs`, and `--exclude-subfolder`."""
    parser.add_argument(
        "--platform",
        type=str,
        nargs="+",
