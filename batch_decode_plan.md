...
4. **Per recursive call target (level 4)** — once-only + all-variants-equivalence (owner's spec; SUPERSEDES the earlier active-path DAG rule):
   - The walk is LEVEL-SYNCHRONOUS over a section's FULL variant set, not a per-variant DFS. A persistent inclusion mask `[#section_variants, #functions]` (a function-id→column hashmap + a bool mask, reused across roots) decides inclusion. The root function's section is seeded at column 0 (`mask[:, 0] = True`) — the root body is always included once, so any deeper call resolving to the root section is already-included (self/mutual recursion never re-splices).
   - Per level, per surviving parent variant: for each call_target the variant DIRECTLY called (its own `per_call_entries`, NOT the section's full union via sibling fallback), resolve the callee section + variant (the J fallback chain is unchanged) and mark `mask[variant, col(callee_section)] = True`. Gates unchanged: EXTERN skipped, `function_section_ptr == 0` skipped, unknown-offset skipped.
   - Post-level: columnwise ALL over the variant axis on the touched columns. A function reached by EVERY variant is NOT emitted and is PRUNED (never expanded deeper) — this generalizes the old `inlined_equivalent_call_targets_only` "called by all ⇒ skip" filter into the default-and-only behaviour (the parameter is retired/always-on). A variant emits a function body only on its FIRST encounter (mask cell False→True) and only if not excluded; diamonds, branch-shared callees, and recursion all dedup to ONE inclusion per variant.
   - Emission is BFS level order (root, then each level's included callees in parent-then-slot order). `path_depth` is the BFS level. Each level-4 entry records `parent_call_target_index` (the parent's call_targets_section row index that pointed here; `None` for the root) and `encounter_category` (`LOCAL_FUNC` for root + LOCAL-inlined; `PLT_FUNC` for PLT-inlined; EXT_FUNC NOT inlined per D3).
   - FLAG-A: a single-variant section's columnwise-ALL is over one row, so it excludes every callee ⇒ single-variant sections splice nothing. FLAG-B: a function reached late by the remaining variants (column becomes all-True at a deeper level) is not included by the late variants and prunes there; variants that reached it earlier keep their inclusion.
