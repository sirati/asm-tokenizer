"""Command-line interface for memmap validation.

This module provides a CLI for validating memmap builder output against
original CSV files, using the shared file discovery logic.
"""
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Validate memory-mapped binary files against CSV data.")
    ...
    logger.info(f"Found {len(matched_pairs)} matched CSV/mapping file pairs to validate")
    ...
    for binary_name, binaries in binaries_by_name.items():
        logger.info(f"\nValidating binary: {binary_name}")
        logger.info(f"  Versions: {len(binaries)}")

        versions = [
            VersionInfo(
                csv_path=binary.path,
        ...
        validator_config = ValidatorConfig(versions=versions, output_dir=config.output_dir, binary_name=binary_name)

        stats = validate_memmap_output(validator_config)
        total_stats.matched_validated += stats.matched_validated
        total_stats.matched_skipped += stats.matched_skipped
        total_stats.unmatched_validated += stats.unmatched_validated
        total_stats.unmatched_skipped += stats.unmatched_skipped
        total_stats.csv_only_matched += stats.csv_only_matched
        total_stats.csv_only_unmatched += stats.csv_only_unmatched
        total_stats.errors.extend(stats.errors)

        if stats.errors:
            logger.error(f"  FAILED: {binary_name} with {len(stats.errors)} error(s)")
        else:
    ...
    logger.info("=" * 80)
    logger.info(f"Total matched functions validated: {total_stats.matched_validated}")
    logger.info(f"Total matched functions skipped: {total_stats.matched_skipped}")
    logger.info(f"Total matched functions in CSV only: {total_stats.csv_only_matched}")
    logger.info(f"Total unmatched functions validated: {total_stats.unmatched_validated}")
    logger.info(f"Total unmatched functions skipped: {total_stats.unmatched_skipped}")
    logger.info(f"Total unmatched functions in CSV only: {total_stats.csv_only_unmatched}")
    logger.info(f"Total errors found: {len(total_stats.errors)}")
    logger.info("=" * 80)
    ...
    if total_stats.errors:
        ...
        logger.error("=" * 80)
        logger.error(f"Found {len(total_stats.errors)} total error(s) across all binaries")
        sys.exit(1)
    else:
        logger.info("\nValidation completed successfully for all binaries!")
