        ...
        for op in insn.operands:
            if op.kind == OperandKind.REG:
                ...
                if not reg.is_absent:
                    ...
                    # PPC ``lis``/``ori`` (and ``lis``/``addi``) high+low
                    # halves build an absolute address; Ghidra attaches
                    # the combined resolved data target as a DATA ref on
                    # the low-half terminal's destination REG operand. The
                    # keep/drop policy (``resolved_target_policy``) owns
                    # the per-ISA pair-terminal allow-list; mirror the
                    # arm32 REG-side consumer exactly.
