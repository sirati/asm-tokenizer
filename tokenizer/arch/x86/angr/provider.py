...
class X86AngrProvider(ArchitectureProvider):
    """Architecture provider for x86 and x64 platforms (angr/Capstone backend)."""
    ...
    def parse_instruction(
        ...
        for op in insn.operands:
            if op.kind == OperandKind.REG:
                ...
                # way to preserve the original guard.
                raise Exception(f"Unsupported x86 operand type: {op.type_int}")
            else:
                # OperandKind.CRX / OperandKind.OTHER -- never produced by
                # the x86 backend (CRX is PPC-only, OTHER is ARM FP/SETEND/
                # SYSREG). Surface as an error to match the legacy
                # `op.type > 3` check.
                raise Exception(f"Unsupported x86 operand type: {op.type_int}")
