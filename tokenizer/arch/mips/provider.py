...
class MIPSProvider(ArchitectureProvider):
    ...
    def parse_instruction(
        ...
        # MIPS has no per-instruction prefix signal; `insn.prefixes`
        # is the empty list and we skip prefix dispatch entirely.

        for op in insn.operands:
            if op.kind == OperandKind.REG:
                reg = op.reg
                insn_tokens.append(vocab_manager.get_registry_token(reg.name, reg.id))
                # MIPS ``lui``/``ori`` (and mips64 ``lui``/``daddi``)
                # high+low halves build an absolute address; Ghidra
                # attaches the combined resolved data target as a DATA
                # ref on the low-half terminal's destination REG operand.
                # The keep/drop policy (``resolved_target_policy``) owns
                # the per-ISA pair-terminal allow-list; mirror the arm32
                # REG-side consumer exactly.
                ...
                raise ValueError(f"Unsupported MIPS operand type: {op.type_int}")
