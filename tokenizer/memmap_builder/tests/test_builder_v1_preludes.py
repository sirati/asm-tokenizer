...
the builder writes carries the new memmap-format-v1 prelude:

  * matched + unmatched ``_sections.csv`` files start with ``# format=N\\n``
  * matched + unmatched ``_index.bin`` files start with ``IDX1`` and the
    16-byte prelude decodes to ``format_version=N`` + ``alignment_shift=2``
...
so no data records are emitted; the prelude bytes are the entire
``_index.bin`` payload. That isolates the prelude concern from the
record encoder.
"""
...
def test_matched_index_bin_starts_with_magic(tmp_path: Path) -> None:
    versions, unified_vocab_path, output_dir = _build_tiny_corpus(tmp_path)
    build_memmap_files(versions, output_dir, "demo", unified_vocab_path)
...
def test_matched_index_prelude_decodes_to_v1(tmp_path: Path) -> None:
    versions, unified_vocab_path, output_dir = _build_tiny_corpus(tmp_path)
    build_memmap_files(versions, output_dir, "demo", unified_vocab_path)

    with open(output_dir / "demo_index.bin", "rb") as fh:
...
def test_index_header_size_is_exactly_16_bytes(tmp_path: Path) -> None:
    """The on-wire prelude width is fixed at 16 bytes; corpus-empty
    builds produce an index file that is exactly that size."""
    versions, unified_vocab_path, output_dir = _build_tiny_corpus(tmp_path)
    build_memmap_files(versions, output_dir, "demo", unified_vocab_path)

    raw_matched = (output_dir / "demo_index.bin").read_bytes()
    ...
    assert len(raw_matched) == INDEX_HEADER_SIZE
    assert len(raw_unmatched) == INDEX_HEADER_SIZE
    # And the reserved + version + magic words all unpack cleanly.
    magic, fmt_v, align, reserved = struct.unpack("<4sIII", raw_matched)
